...
# CHATROOM FORMAT
class ChatRoom:
    def __init__(self, roomname, port, password, limit):
...
class CreateRoom:
    def __init__(self):
        self.room = None
        self._createWindow = ctk.CTk
        self._createWindow.geometry('600x500')
        self._createWindow.title('Create ChatRoom')
        self._createFrame = ctk.CTkFrame(master=self._createWindow)
        self._createFrame.pack(pady=12, padx=20, fill='both', expand=True)
        self.createLabel = ctk.CTkLabel(master=self._createFrame, text='Create Chatroom', font=('Roboto', 30),
                                        height=35, width=400, anchor='w')
        self.createLabel.pack(pady=20, padx=20)

        self.roomnameEntry = ctk.CTkEntry(master=self._createFrame, placeholder_text='Room Name', height=35, width=400,
                                          font=('Helvetica', 20))
        self.roomnameEntry.pack(pady=12, padx=20)

        self.portEntry = ctk.CTkEntry(master=self._createFrame, placeholder_text='Port (1000-9999)', height=35,
                                      width=400, font=('Helvetica', 20))
        self.portEntry.pack(pady=12, padx=20)

        self.clientEntry = ctk.CTkEntry(master=self._createFrame, placeholder_text='Number of Clients', height=35,
                                        width=400, font=('Helvetica', 20))
        self.clientEntry.pack(pady=12, padx=20)

        self._passEntry = ctk.CTkEntry(master=self._createFrame, placeholder_text='Password', height=35, show='*',
                                       width=400, font=('Helvetica', 20))
        self._passEntry.pack(pady=12, padx=20)

        self._createButton = ctk.CTkButton(master=self._createFrame, text='Create', height=50, width=250,
    ...
    def get_values(self):
        ...
        self.room = ChatRoom(roomname=roomname, port=port, password=password, limit=limit).send_to_db()
        print(self.room)
...
class CTkChatRoomTile:
    def __init__(self, master, username, time):
        _frame = ctk.CTkFrame(master=master, height=60)
        _frame.pack(pady=10, padx=20)
        label = ctk.CTkLabel(master=_frame, text=username, font=('Roboto', 30), anchor='nw', height=40, width=500)
        label.pack(padx=10, pady=5)
        status_lab = ctk.CTkLabel(master=_frame, text=time, anchor='nw', height=30, width=500)
...
class App(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.add_user_image = ctk.CTkImage(light_image=Image.open(os.path.join(image_path, "add_user_dark.png",
                                                                               dark_image=Image.open(
                                                                                   os.path.join(image_path,
                                                                                                "add_user_light.png"),
                                                                                   size=(20, 20)))))
        self.add_image = ctk.CTkImage(Image.open(os.path.join(image_path, "add_chatroom.png"), size=(20, 20)))

        self.navigation_frame = ctk.CTkFrame(self, corner_radius=0)
        self.navigation_frame.grid(row=0, column=0, sticky="nsew")
        self.navigation_frame.grid_rowconfigure(4, weight=1)
        self.navigation_frame_label = ctk.CTkLabel(self.navigation_frame, text="SOCIAL SQUAD", image=self.logo_image,
        ...
        us = LoginUser(name="Mohamed Sameer", username='sameer', password='my_secret', email='thismyemail@domaim.com')
...
# CHAT BUBBLE
class CTkChatBubble:
    def __init__(self, master, username, message, time):
        ...
        self._username = ctk.CTkLabel(master=self._Buble, text=username, text_color=('black', 'white'),
                                      height=15, width=350, anchor='nw', font=('monospace', 10, 'bold'))
...
class ChatUI:
    def __init__(self, chatroomname):
        ...
        self.roomlabel = ctk.CTkLabel(master=self.infoFrame, text=chatroomname, height=60, width=550,
                                      anchor='w', font=('Helvetica', 25))
        self.roomlabel.pack(padx=10)
